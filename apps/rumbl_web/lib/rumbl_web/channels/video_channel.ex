defmodule RumblWeb.VideoChannel do
  ...
  def join("videos:" <> video_id, params, socket) do
    ...
    annotations =
      ...
      |> Enum.map(&AnnotationHTML.render_annotation(%{annotation: &1}))
  ...
  defp broadcast_annotation(socket, user, annotation) do
    broadcast!(socket, "new_annotation", %{
      id: annotation.id,
      user: RumblWeb.UserHTML.render_user(user),
  ...
  defp compute_additional_info(annotation, socket) do
    for result <- InfoSys.compute(annotation.body, limit: 1, timeout: 10_000) do
      ...
      case Multimedia.annotate_video(backend_user, socket.assigns.video_id, attrs) do
        {:ok, info_ann} ->
